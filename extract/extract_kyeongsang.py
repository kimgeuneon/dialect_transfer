...
# JSON 파일들이 저장된 경로
json_folder_path = './2'  # JSON 파일이 저장된 폴더 경로
output_file_path = 'kyeongsang_data.json'  # 최종 병합된 JSON 파일 이름
            ...
            try:
                ...
                with open(file_path, 'r', encoding='utf-8') as json_file:
                    ...
                    if isinstance(data.get('transcription'), dict):
                        ...
                        source = transcription.get('standard', '')  # 기존 standard를 source로 변환
                        target = transcription.get('dialect', '')  # 기존 dialect를 target으로 변환 

                        # dialect와 standard가 같으면 제외 / 경상도는 억양 문제로 동일한 문장 존재
                        if source != target:
                            # 변환된 데이터를 리스트에 추가
                            merged_data.append({
                                "source": source,
